...
def text_input(text, player_room, version = 'main', useGraphics=False):
    text = text.lower()
    fullText = text
    text = text.split()
    try:
        ...
        if version == 'dev' and useGraphics:
            type_effect(text, useGraphics=True)
            #print(text)
            if text == ['input', 'test']:
                gottenText = type_effect('enter some input please: ', useGraphics=True, getInput=True)
                while inputText:
                    print(inputText)
            print(gottenText)
